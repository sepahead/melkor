...
CloudInspection inspectCloud(const SplatData& cloud) {
    ...
    for (std::size_t index = 0; index < cloud.size(); ++index) {
        ...
        const std::array<float, 3> squared{scale.x * scale.x, scale.y * scale.y,
                                          scale.z * scale.z};
        ...
        } else if (std::any_of(squared.begin(), squared.end(),
                               [](float value) {
...
CloudInspection inspectLegacyCloud(const GaussianCloud& cloud) {
    CloudInspection result;
    result.splat_count = cloud.size();
